...
drop table h_po cascade constraint purge;
drop table d_po cascade constraint purge;
...
(
	...
	complain varchar2(10) constraint com_beli not null
);
...
(
	...
	harga number(9) constraint db_harga not null,
...
(
	...
	harga number(9) constraint dj_harga not null,
...
--HPO
create or replace trigger tIdPo
before insert on h_po
for each row
declare
	angka number;	
	con varchar2(5);
begin
	select count(*) into angka from h_po;	
	con := 'PO' || lpad(angka+1,3,0);
	:new.id_po := con;
end;
/
show err;
...
insert into d_jual values('NJ001','DI001',5,'box',15000);
insert into d_jual values('NJ001','PD001',10,'biji',15000);
insert into d_jual values('NJ002','PD001',10,'biji',15000);
insert into d_jual values('NJ003','DI001',5,'strip',15000);
insert into d_jual values('NJ003','PD001',10,'strip',15000);
...
insert into h_beli values('','S_FA001',to_date('01-01-2019','DD-MM-YYYY'),20000,'order');
insert into h_beli values('','S_FA001',to_date('01-01-2019','DD-MM-YYYY'),20000,'order');
insert into h_beli values('','S_FA002',to_date('01-01-2019','DD-MM-YYYY'),20000,'order');
insert into h_beli values('','S_FA002',to_date('01-01-2019','DD-MM-YYYY'),20000,'order');
              
insert into d_beli values('NB001','DI001',5,'box',10000);
insert into d_beli values('NB001','PD001',10,'strip',10000);
insert into d_beli values('NB002','PD001',10,'biji',10000);
insert into d_beli values('NB003','DI001',5,'box',10000);
insert into d_beli values('NB003','PD001',10,'biji',10000);
insert into d_beli values('NB004','PD001',7,'biji',10000);

insert into h_po values('','NB001','S_FA001',to_date('01-01-2019','DD-MM-YYYY'),20000,'order');

insert into d_po values('PO001','DI001',5,'box',10000);
...
tambah tabel po atribut(kode po(pk), id_hbeli, status)
tambah tabel po_detail atribut(product id, quantity, kode po)

create table h_po
(
	id_po varchar2(5) constraint pk_h_po primary key,
	nota_beli varchar2(10) constraint fk_po_beli_nota references h_beli(nota_beli),
	id_supplier varchar2(7) constraint fk_h_po_id_supplier references supplier(id_supplier),
	tanggal date constraint nn_h_po_tanggal not null,
	total number(9) constraint nn_h_po_total not null,
	status varchar2(255)
);

create table d_po
(
	id_po varchar2(5) constraint fk_d_po not null,
	id_barang varchar2(6) constraint po_id_barang not null,
	qty number(5) constraint ch_po_jual_qty check(qty >= 0),
	jenis varchar2(10) constraint ch_dpo_jenis check(jenis = 'box' or jenis = 'strip' or jenis = 'biji'),
	harga number(9) constraint dpo_harga not null
);
